...
public unsafe partial interface ISession
{
    ...
    ///
    ///     When <paramref name="source"/> is non-null the module is compiled from that blob, and
    ///     <paramref name="path"/> is used only for diagnostics and as the base directory for
    ///     resolving relative <c>import</c>s.
    ///
    ///     When <paramref name="source"/> is null and <paramref name="path"/> names a readable file,
    ///     the module is loaded from that file's contents. If the file cannot be read the call
    ///     returns <c>null</c> and writes a <c>CannotOpenFile</c> diagnostic to
    ///     <paramref name="diagnostics"/>.
    ///
    ///     To load a module by name via the search paths, use
    ///     <see cref="LoadModule(string, out ISlangBlob?)"/> instead.
    /// </summary>
    [PreserveSig]
    [return: MarshalUsing(typeof(NoFreeComInterfaceMarshaller<IModule>))]
