...
Copy-Item -Path "$env:TEMP\slang\windows-aarch64\bin\slang.dll"     -Destination ".\runtimes\win-arm64\native\slang.dll"        -Force
Copy-Item -Path "$env:TEMP\slang\windows-x86_64\bin\slang.dll"      -Destination ".\runtimes\win-x64\native\slang.dll"          -Force
Copy-Item -Path "$env:TEMP\slang\macos-x86_64\lib\libslang.dylib"   -Destination ".\runtimes\osx-x64\native\libslang.dylib"     -Force
Copy-Item -Path "$env:TEMP\slang\macos-aarch64\lib\libslang.dylib"  -Destination ".\runtimes\osx-arm64\native\libslang.dylib"   -Force
Copy-Item -Path "$env:TEMP\slang\linux-aarch64\lib\libslang.so"     -Destination ".\runtimes\linux-arm64\native\libslang.so"    -Force
Copy-Item -Path "$env:TEMP\slang\linux-x86_64\lib\libslang.so"      -Destination ".\runtimes\linux-x64\native\libslang.so"      -Force
